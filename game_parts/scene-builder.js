...
export function buildOfficeScene({ scene, renderer }) {
  // ── Lighting ─────────────────────────────────────────────────
  ...
  function box(w, h, d, mat, rx = 0, ry = 0, rz = 0) {
    const key = `${w}|${h}|${d}`;
    if (!box._geoCache) box._geoCache = new Map();
    if (!box._geoCache.has(key)) {
      box._geoCache.set(key, new THREE.BoxGeometry(w, h, d));
    }
    const m = new THREE.Mesh(box._geoCache.get(key), mat);
  ...
  return {
    ...
    player,
    npcs: [bossNPC, col1NPC, col2NPC],
    animatePlayerRig,
    avatarTextures: NPC_AVATAR_TEX,
    boxGeoCache: box._geoCache,
  };
}
